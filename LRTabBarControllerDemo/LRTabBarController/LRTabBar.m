...
- (void)setTab_selected:(LRTab *)tab_selected{
...
- (void)layoutSubviews {
    ...
    for (LRTab *tab in _arr_tabs) {
        ...
        if ([_arr_tabs indexOfObject:tab] == 0) {
            ...
            if(tab.isSpecialTab){
                tab.frame = CGRectMake(rect.origin.x + kInterTabMargin, rect.origin.y- ((tab.SpecialTabHeight - self.bounds.size.height) > 0 ? (tab.SpecialTabHeight - self.bounds.size.height) : 0), dTabWith, (tab.SpecialTabHeight > self.bounds.size.height ? tab.SpecialTabHeight : self.bounds.size.height));
                tab.tab_selectedBgImageName = nil;
